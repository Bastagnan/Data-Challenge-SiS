...
def temporal_loss(motion, target):
    # motion, target: (B, T, num_joints, 3)
    velocity_motion = motion[:, 1:] - motion[:, :-1]
    velocity_target = target[:, 1:] - target[:, :-1]
    vel_loss = F.mse_loss(velocity_motion, velocity_target)

    acceleration_motion = velocity_motion[:, 1:] - velocity_motion[:, :-1]
    acceleration_target = velocity_target[:, 1:] - velocity_target[:, :-1]
    acc_loss = F.mse_loss(acceleration_motion, acceleration_target)
...
def train(model, 
    ...
    for epoch in range(num_epochs):
        ...
        for motions, texts in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            loss = criterion(pred_motion, gt_motion) + temporal_loss(gt_motion.view(gt_motion.size(0), 100, 22, 3), pred_motion.view(gt_motion.size(0), 100, 22, 3))
            optimizer.zero_grad()
            loss.backward()
